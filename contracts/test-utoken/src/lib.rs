...
mod tests {
    ...
    fn init() -> (VMContextBuilder, AccountId, Contract) {
        ...
        // init the contract
        let contract = Contract::new_default_meta(_contract_account.clone(), "name".to_string(), "symbol".to_string(), TOTAL_SUPPLY.into());
    ...
    fn test_transfer() {
        ...
        testing_env!(context.build());
        let mut contract = Contract::new_default_meta(accounts(2).into(), "name".to_string(), "symbol".to_string(), TOTAL_SUPPLY.into());
        testing_env!(context
        ...
        contract.ft_transfer(accounts(1), transferred_tokens.into(), Some("you have received some tokens bro".to_string()));
