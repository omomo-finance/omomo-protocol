...
fn repay_more_dtoken_fixture() -> (ContractAccount<dtoken::ContractContract>, ContractAccount<controller::ContractContract>, ContractAccount<test_utoken::ContractContract>, UserAccount) {
    let (dtoken, controller, utoken, user) = base_more_dtoken_fixture();
...
fn scenario_supply() {
    ...
    assert_eq!(user_balance, 10.to_string(), "User balance should be 10");


    // checking after supplying -- should be 10, bc of increasing the number whilst supply cross contract call
    let dtoken_total_supplies: Balance = view!(
        dtoken.get_total_supplies()
    ).unwrap_json();

    assert_eq!(dtoken_total_supplies, 10, "dtoken total supplies should be 10");
    ...
    let user_balance: u128 = view_balance(&controller, Supply, user.account_id(), dtoken.account_id());
    assert_eq!(user_balance, 10, "Balance on controller should be 10");
}
...
fn scenario_withdraw() {
    ...
    // 20 (supplied) - 10 (withdraw) = 10 remaining
    let dtoken_total_supplies_after_withdraw: Balance = view!(
        dtoken.get_total_supplies()
    ).unwrap_json();

    assert_eq!(dtoken_total_supplies_after_withdraw, 10, "dtoken total supplies should be 10");
...
fn scenario_repay_more_than_borrow() {
    ...
    assert_eq!(user_balance, 7.to_string(), "As it was borrowed 10 tokens and repayed 13 tokens (rate 1.3333), balance should be 7");
