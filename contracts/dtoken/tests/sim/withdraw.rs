...
fn withdraw_with_borrow_on_another_dtoken_fixure() -> (
    ...
    call!(
        ...
        utoken2.mint(dtoken2.account_id(), U128(1000000)),
        0,
        100000000000000
    );
...
fn scenario_withdraw_with_borrow_on_another_dtoken() {
    ...
    call!(user, dtoken2.withdraw(U128(50000)), deposit = 0).assert_success();

    let user_balance: u128 =
        view_balance(&controller, Supply, user.account_id(), dtoken2.account_id());
    assert_eq!(user_balance, 57170, "Balance should be 57170");
    ...
    assert_eq!(
        dtoken_balance,
        U128(99999942830),
        "After withdraw balance should be 42830"
    );
}
