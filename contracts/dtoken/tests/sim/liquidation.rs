...
fn scenario_liquidation_success() {
    ...
    let weth_ft_balance_of_for_dweth: U128 = view!(weth.ft_balance_of(dweth.account_id())).unwrap_json();

    assert_eq!(
        Balance::from(weth_ft_balance_of_for_dweth), (MINT_BALANCE - BORROWER_BORROW + Balance::from(amount.clone())),
    ...
    let revenue_amount: Balance =  ( 10500 * Balance::from(amount.clone()) * START_PRICE)/ (CHANGED_PRICE * RATIO_DECIMALS);

    assert_eq!(
        user_borrows, borrow_balance.clone(),
        "Borrow balance on dtoken should be {}",
    ...
    assert_eq!(
        user_balance, revenue_amount.clone(),
        "Supply balance on dtoken should be {}",
        revenue_amount.clone()
    );

    let borrower_dwnear_balance: U128 = view!(dwnear.ft_balance_of(borrower.account_id())).unwrap_json();

    assert_eq!(
        Balance::from(borrower_dwnear_balance), BORROWER_SUPPLY - revenue_amount.clone(),
        "Borrower balance on dtokn ft should be {}",
        BORROWER_SUPPLY - revenue_amount.clone()
    );

    let liquidator_dwnear_balance: U128 = view!(dwnear.ft_balance_of(liquidator.account_id())).unwrap_json();

    assert_eq!(
        Balance::from(liquidator_dwnear_balance), revenue_amount.clone(),
