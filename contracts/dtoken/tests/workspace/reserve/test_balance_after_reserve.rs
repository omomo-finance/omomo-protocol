...
async fn create_custom_market(owner: &Account, worker: &Worker<Sandbox>) -> Result<workspaces::Contract, workspaces::error::Error>{
    ...
    let _ = market.call("new_with_config")
        .args_json(json!({ "owner_id":  owner.id(), "underlying_token_id": underlying_token.id(),
